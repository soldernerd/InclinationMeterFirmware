...
static void task_adc(void)
{
     ...
     * downstream readers (drv_tmp236, drv_lm35, svc_battery, optional
     * VREFINT cross-check) run on their own slower periods and just
     * sample the latest hal_adc results. */
    if (hal_adc_is_ready()) {
        hal_adc_start();
    }
}
...
static void task_temperature(void)
{
    ...
    lm35_data_t ext_data;
    if (drv_lm35_get_result(&ext_data) == DRV_OK) {
        g_system_state.lm35_temp_cdeg = ext_data.temp_cdeg;
    }
    /* drv_tmp236_start_read/drv_lm35_start_read both just call
     * hal_adc_start; safe to call even if a scan is already running --
     * hal_adc_start no-ops in that case. Both channels come from the
     * same shared scan (see hal_adc.h's adc_results_t), so either call
     * alone would suffice, but each driver triggering its own read
     * keeps them independently correct rather than one silently relying
     * on the other. */
...
static void task_api_meas_subs(void) { svc_api_measurement_subscriptions_update(); }
 ...
 * API-layer consumer of the output ring is gone. The v2 equivalent is
 * explicitly NOT stage 2 (2026-08-19 rule: only single-data-point
 * Measurements/Settings/Calibrations are built "out of the box" right
 * now; the high-rate per-cycle displacement stream is a "bigger item"
 * like Topic groups/Bulk, deferred until actually needed). Until then,
...
static SchedulerEntry s_tasks[] = {
    ...
    { task_displacement, 0,                   0 },
    { task_ui,          0,                    0 },
    { task_display,     0,                    0 },
    { task_leds,        DEFAULT_TASK_LED_MS, 0 },
    { task_bme280,      DEFAULT_TASK_BME280_MS, 0 },
    { task_api_meas_subs, SYSTICK_PERIOD_MS, 0 },   /* every tick -- see
                                                        * svc_api.h's doc
                                                        * comment on
                                                        * svc_api_measurement_
...
void app_scheduler_reload_periods(void)
{
    ...
    s_tasks[11].period_ms = SYSTICK_PERIOD_MS;   /* displacement -- must drain every
                                                     * tick, not just at task_sensors_ms's
                                                     * rate: at ~2.6 kHz carrier-cycle
                                                     * production, the 64-entry input ring
                                                     * buffer (~24.6 ms of headroom) would
                                                     * already overrun partway through a
                                                     * single 100 ms task_sensors_ms period,
                                                     * let alone between ticks; see
                                                     * svc_displacement.c */
     ...
     * s_tasks[16] (Measurements subscription delivery) periods are the
     * fixed literals set in the table above — not user/BLE-configurable
     * like the others (DeviceSettings has no room left for another field
     * — see config.h's DEFAULT_TASK_UART_MS comment). s_tasks[16]
     * specifically must stay fixed at SYSTICK_PERIOD_MS regardless — see
     * svc_api.h's doc comment on
